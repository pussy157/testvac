...
#include <string>
#include <map>
#include "../examples/example_win32_directx11/custom_widgets.hpp"
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::anim::active : hovered ? c::text::label::hovered : c::text::label::default, g.IO.DeltaTime * 6.f);

	if (!is_multiline)
	{
		GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, c::anim::default, c::elements::rounding);
	...
	if (render_cursor || render_selection)
	{
			...
			for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
			{
				...
				else
				{
					...
					if (rect.Overlaps(clip_rect))
						window->DrawList->AddRectFilled(rect.Min, rect.Max, GetColorU32(c::anim::active, 0.2f), c::elements::rounding);
				}
		...
		{
				ImU32 col = is_displaying_hint ? c::label::default: c::label::active;
	...
	it_anim->second.label = ImLerp(it_anim->second.label, CalcTextSize(buf_display).x < 1.f ? c::text::label::default: utils::GetColorWithAlpha(c::text::label::default, 0.f), g.IO.DeltaTime * 6.f);
